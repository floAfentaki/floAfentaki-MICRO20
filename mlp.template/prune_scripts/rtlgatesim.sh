...
#vsim -c -t 1ps -sdfmax /${ENV_DUT_NAME}/=./gate/${ENV_TOP_DESIGN}.sdf -do ./scripts/gatesim.tcl work_gate.${ENV_TB_NAME}
vsim -c -t 1ps -do ./scripts/gatesim.tcl work_gate.${ENV_TB_NAME}
exstatus="$?"
if [ "$exstatus" -ne 0 ]; then
